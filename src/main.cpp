...
unsigned int nStakeTargetSpacing = 60;			        // 20 sec block spacing for PoS
unsigned int nWorkTargetSpacing = 60;                  // 120 sec block spacing for PoW
...
const string strMessageMagic = "WashingtonCoin Signed Message:\n";
...
static int FINAL_POW_HEIGHT = 200000;
static int FINAL_POW_COUNT = 64800;
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, const CBlockIndex* pindex)
{
	...
	uint256 prevHash = pindex->pprev->GetBlockHash();
	std::string cseed_str = prevHash.ToString().substr(12, 7);
	const char* cseed = cseed_str.c_str();
	long seed = hex2long(cseed);
	int rand = GenerateMTRandom(seed, 512);

	nSubsidy = (512 + rand) * COIN;
	nSubsidy >>= (nPoWHeight / 21600);
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, const CBlockIndex* pindex)
{
	...
	{
		nSubsidy = 16 * nRewardCoinYear * nCoinAge / 365;
	}
	else if(nPoSHeight < 2 * YEARLY_POS_BLOCK_COUNT)	
	{
		nSubsidy = 8 * nRewardCoinYear * nCoinAge / 365;
	}
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "WashingtonCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "WashingtonCoin: a coin that honors George Washington!";
        CTransaction txNew;
        txNew.nTime = 1476131556;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1476131566;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3438270;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x305097b364e2d749a64b1dcf9bb20dec55938410cef47c562383a65d45da1598"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xdb, 0xfb, 0xc3 };
